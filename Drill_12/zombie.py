...
class Zombie:
    ...
    def find_ball(self):
        balls = main_state.balls
        ball = None
        standard = 100.0
        for i in balls:
            distance = (i.x - self.x) ** 2 + (i.y - self.y) ** 2
            if standard <= distance:
                standard = distance
                ball = i

        if standard < (PIXEL_PER_METER * 10) ** 2:
    ...
    def build_behavior_tree(self):
        ...
        run_by_player_node = LeafNode("Run by Player", self.run_by_player)
        find_ball_node = LeafNode("Find Ball", self.find_ball)

        chase_node = SequenceNode("Chase")
        chase_node.add_children(find_player_node, move_to_player_node)

        run_node = SequenceNode("Run")
        run_node.add_children(find_player_node, run_by_player_node)

        wander_chase_node = SelectorNode("WanderChase")
        wander_chase_node.add_children(chase_node, wander_node)

        run_find_node = SelectorNode("RunFind")
        run_find_node.add_children(run_node, wander_node)

        Behavior_node = SelectorNode("WanderChase")
        Behavior_node.add_children(wander_chase_node, run_find_node)

        self.bt = BehaviorTree(Behavior_node)
